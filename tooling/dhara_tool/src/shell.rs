...
fn prompt_for(registry: &CommandRegistry, current_section: &str) -> String {
    ...
    registry
        ...
        .unwrap_or_else(|| format!("dhara:{current_section}> "))
}
...
mod tests {
    ...
    fn prompt_uses_section_prompt_when_registered() {
        ...
        assert_eq!(prompt_for(&registry, "verify"), "dhara:verify> ");
    }

    #[test]
    fn prompt_falls_back_for_unknown_sections() {
        let registry = CommandRegistry::new();
        assert_eq!(prompt_for(&registry, "defs"), "dhara:defs> ");
    }

    #[test]
    fn prompt_for_root_uses_root_prompt() {
        let registry = CommandRegistry::new();
        assert_eq!(prompt_for(&registry, "root"), "dhara> ");
    }
}
