...
`rheo_storage_ffi` exposes a path-based native C ABI over `rheo_storage`.

It is the supported interop layer for `bindings/dotnet/Rheo.Storage` and for any
...
- The crate stays intentionally thin; filesystem behavior remains owned by `rheo_storage`.

## Logging

Hosts can register a logger callback through `rheo_register_logger`. Each callback
...
- Core runtime: <https://github.com/D-Naveenz/rheo_storage/tree/main/rheo_storage>
- .NET wrapper: <https://github.com/D-Naveenz/rheo_storage/tree/main/bindings/dotnet/Rheo.Storage>
