...
- Keep the generic `RPKG` v2 container in a shared crate, `rheo_rpkg`, while letting `rheo_storage` remain a read-side consumer of filedefs packages and `rheo_tool_rheo_storage` remain the write-side producer from TrID XML sources.
